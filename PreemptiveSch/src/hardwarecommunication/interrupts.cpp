...
{
    // return (uint32_t)interruptManager->taskManager->robinScheduler((CPUState*)esp);
    return (uint32_t)interruptManager->taskManager->priorityScheduler((CPUState*)esp);
...
uint32_t InterruptManager::DoHandleInterrupt(uint8_t interrupt, uint32_t esp)
{
    ...
    {
        // esp = (uint32_t)taskManager->robinScheduler((CPUState*)esp);
        esp = (uint32_t)taskManager->priorityScheduler((CPUState*)esp);
