...
//overload a new constructor for creating task with a priorty attribute
Task::Task(GlobalDescriptorTable *gdt, void entrypoint(), myos::TaskPriority prt = LOW)
{
...
CPUState* TaskManager::priorityScheduler(CPUState* cpustate) {
        if(totTaskCount <= 0)
            return cpustate;
        ...
        int to_be_scheduled = -1;
        int highest_priority = HIGH;
        for (int i = 0; i < totTaskCount; i++) 
        {
          
            if(tasks[i].taskState == READY && tasks[i].priority <= highest_priority) {
                to_be_scheduled = i;
                highest_priority = tasks[i].priority;
            }
        }

        if(to_be_scheduled == -1) {
            // No ready task with higher priority than the current one
            return cpustate;
        }
        else{
            currentTask = to_be_scheduled;
